...
import { Row, Col, Card, CardBody, Badge, Input, Modal, ModalHeader, ModalBody, Nav, NavItem, NavLink, TabContent, TabPane } from "reactstrap";
...
const AdminBots = (props) => {
  ...
  useEffect(() => {
    props.setBreadcrumbItems("All Bots", [{ title: "Admin", link: "#" }, { title: "All Bots", link: "#" }]);
    getAdminAllBots()
  ...
  return (
    <React.Fragment>
      <Row>
        ...
        <Col xl={3}>
          <Card style={{ position: "sticky", top: 80 }}>
            <CardBody>
              ...
              {bots.map((bot) => (
                <div key={bot.bot_id} onClick={() => selectBot(bot)}
                  style={{ padding: "12px", borderRadius: 10, marginBottom: 8, cursor: "pointer", border: `2px solid ${selectedBot?.bot_id === bot.bot_id ? bot.primary_color || "#556ee6" : "#e9ecef"}`, background: selectedBot?.bot_id === bot.bot_id ? `${bot.primary_color}11` : "#fff", transition: "all 0.2s" }}>
                  <div className="d-flex align-items-center gap-2">
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: bot.primary_color || "#556ee6", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      {bot.logo_url
        ...
        <Col xl={9}>
          {!selectedBot ? (
            <Card><CardBody className="text-center py-5 text-muted"><i className="mdi mdi-cursor-pointer font-size-36 d-block mb-2"></i>Select a bot from the left to view its data</CardBody></Card>
          ) : (
            <>
              {/* Bot Header */}
              <Card className="mb-3">
                <CardBody>
                  <div className="d-flex align-items-center gap-3">
                    <div style={{ width: 44, height: 44, borderRadius: 10, background: selectedBot.primary_color || "#556ee6", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              <Card>
                <CardBody>
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <Nav tabs>
                      <NavItem>
                        <NavLink className={activeTab === "leads" ? "active" : ""} onClick={() => { setActiveTab("leads"); setSearch(""); }} style={{ cursor: "pointer" }}>
                          Leads <Badge color="primary" className="ms-1">{leads.length}</Badge>
                        </NavLink>
                      </NavItem>
                      <NavItem>
                        <NavLink className={activeTab === "conversations" ? "active" : ""} onClick={() => { setActiveTab("conversations"); setSearch(""); }} style={{ cursor: "pointer" }}>
                    ...
                    <TabPane tabId="conversations">
                      {dataLoading ? <div className="text-center py-4"><div className="spinner-border text-primary" /></div> : (
                        <div className="table-responsive">
                          <table className="table table-centered table-hover table-nowrap mb-0">
                            <thead className="table-light">
                              <tr><th>#</th><th>Session ID</th><th>Messages</th><th>Started</th><th>Action</th></tr>
                            </thead>
                            <tbody>
      ...
      {/* Chat History Modal */}
      <Modal isOpen={historyModal} toggle={() => setHistoryModal(false)} size="lg" scrollable>
                ...
                {messages.map((msg, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "flex", justifyContent: isUser ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 8 }}>
                      {!isUser && <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#556ee6", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><i className="mdi mdi-robot" style={{ color: "#fff", fontSize: 16 }}></i></div>}
                      <div style={{ maxWidth: "72%", padding: "10px 14px", borderRadius: isUser ? "18px 18px 4px 18px" : "18px 18px 18px 4px", background: isUser ? "#556ee6" : "#f8f9fa", color: isUser ? "#fff" : "#495057", fontSize: 14 }}>
                        <div style={{ whiteSpace: "pre-wrap" }}>{msg.content}</div>
                        {msg.timestamp && <div style={{ fontSize: 10, opacity: 0.65, marginTop: 4, textAlign: "right" }}>{new Date(msg.timestamp).toLocaleTimeString("en-IN")}</div>}
                      </div>
                      {isUser && <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#34c38f", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><i className="mdi mdi-account" style={{ color: "#fff", fontSize: 16 }}></i></div>}
                    </div>
                  );
                })}
              </div>
      ...
      <Modal isOpen={configModal} toggle={() => setConfigModal(false)} size="xl" scrollable>
        ...
        <ModalBody>
          {selectedBot && <BotConfig botId={selectedBot.bot_id} setBreadcrumbItems={() => {}} />}
        </ModalBody>
      </Modal>
    </React.Fragment>
  );
